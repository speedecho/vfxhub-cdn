...
function signup_popup(){
  ...
  $(document).on('click', ".go-login", function (event) {
    ...
    if (vfxhub.tencent_captcha.is == 1) {
      // 直接生成一个验证码对象
      var tx_captcha = new TencentCaptcha(vfxhub.tencent_captcha.appid, function (res) {
        console.log(res)
        if (res.ret === 0) {
          $.post(vfxhub.ajaxurl, {
              "action": "tencentcaptcha",
              "appid": vfxhub.tencent_captcha.appid,
              "Ticket": res.ticket,
              "Randstr": res.randstr
          }, function(data) {
              $.post(vfxhub.ajaxurl, {
  ...
  $(document).on('click', ".go-register", function (event) {
    ...
    if (vfxhub.tencent_captcha.is == 1) {
      // 直接生成一个验证码对象
      var tx_captcha = new TencentCaptcha(vfxhub.tencent_captcha.appid, function (ress) {
...
function share_pop() {
  ...
  $('[etap="share"]').on('click', function(){
    ...
    switch(to){
        case 'qq':
            url = 'http://connect.qq.com/widget/shareqq/index.html?url='+share.url+'&desc='+share.desc+'&summary='+share.title+'&site='+vfxhub.site_name+'&pics='+share.pic
            break;
        case 'weibo':
...
function userinit(){
  ...
  $('.unset-bind').on('click', function(){
      ...
      Swal.fire({
        ...
        if (result.value) {
          $.post(vfxhub.ajaxurl,{action: 'unset_open_oauth',unsetid: unsetid},
              function (data) {
                  if (data == 1) {
                    _this.html(deft)
  ...
  $('.click-qiandao').on('click', function(){
    ...
    $.post(vfxhub.ajaxurl,{action: 'user_qiandao'},
          function (data) {
              if (data.status == 1) {
                _this.html(deft)
  ...
  if (document.getElementById("editor")) {
    ...
    editor.customConfig.uploadImgServer = vfxhub.ajaxurl
    // 将图片大小限制为 2M
...
function widget_ri(){
  ...
  $(".click-pay").on("click",function(){
    ...
    Swal.fire({
      title: '请选择支付方式',
      html:vfxhub.pay_type_html.html,
      showConfirmButton: false,
      width: 300,
      showCloseButton: true,
      onBeforeOpen: () => {
        ...
        if (alipay) {
          alipay.addEventListener('click', () => {
            to_pay_post(vfxhub.pay_type_html.alipay,'other',post_id,nonce)
          })
        }
        if (weixinpay) {
          weixinpay.addEventListener('click', () => {
            to_pay_post(vfxhub.pay_type_html.weixinpay,'other',post_id,nonce)
          })
        }
        if (yecpay) {
...
function megaMenu() {
  ...
  $(document).on('click', ".tap-dark", function (event) {
    ...
    $.ajax({
        ...
        dataType: 'html',
        data: {
            "is_vfxhub_dark": $('body').hasClass('vfxhub-dark') === true ? '0' : '1',
            action: 'tap_dark'
        },
    })
...
function pagination() {
  ...
  if (body.hasClass('pagination-infinite_button')) {
    ...
    wrapper.on('request.infiniteScroll', function (event, path) {
      button.html(vfxhub.infinite_loading);
    });

    wrapper.on('load.infiniteScroll', function (event, response, path) {
      button.html(vfxhub.infinite_load);
    });
  }
...
function fancybox() {
  'use strict';
  $(function() {
    if (vfxhub.is_singular == 0) {return false;}
...
function notify(){
  'use strict';
  $(function() {
    if (vfxhub.site_notice.is == 0) {return false;}
    if($.cookie("cao_notice_cookie")==null){
      Swal.fire({
        html: vfxhub.site_notice.html,
        background: vfxhub.site_notice.color,
        showConfirmButton: false,
        width: 560,
        padding: '0',
        allowOutsideClick:false,
